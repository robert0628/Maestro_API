...
def auto_graph_generate_nodes(nodes, json_file):
    ...
    for node in nodes:
        ...
        network = IPNetwork('/'.join([node['ip'], '24']))
...
def convert_to_json(content):
    '''
        Ouput template (from https://www.tunnelsup.com/packet-captures-on-cisco-asa/):
            HH:MM:SS.ms [ether-hdr] src-addr.src-port dest-addr.dst-port: tcp-flags [header-check] 
            [checksum-info] sequence-number ack-number tcp-window urgent-info tcp-options
    '''
...
def process_live_data(response_data, live_data, timeframe=10000):
    ...
    for link in response_data["links"]:
        if link["value"] > 0:
            ...
            #TODO:check values (75 value currentTrafficSize 54/1441??)
            link['trafficType'] = 'TCP'
            #TODO:there's multiple types of traffic for IP1:IP2 connection ???
            link['absolutePath'] = [link['source'], link['target']]
